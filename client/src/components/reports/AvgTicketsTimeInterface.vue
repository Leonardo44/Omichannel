<template>
    <v-layout row wrap>
      ...
      <v-dialog v-model="resultCont" fullscreen hide-overlay transition="dialog-bottom-transition">
        <form ref="frmReportPDF" method="POST" action="http://172.16.11.172:9000/api/account/avg_duration_tickets_interface/pdf" target="__blank">
          <input type="hidden" name="data" :value="JSON.stringify(reportData)">
        </form>

        <form ref="frmReportExcel" method="POST" action="http://172.16.11.172:9000/api/account/avg_duration_tickets_interface/excel" target="__blank">
          <input type="hidden" name="data" :value="JSON.stringify(reportData)">
        </form>
        <v-card>
