    ...
    <v-layout row wrap>
      ...
      <v-flex xs12>
        <v-form ref="form" v-model="frmIsValid" lazy-validation>
          <h2 :class="colors.primary.text + ' text-xs-center text-sm-center text-md-center text-lg-center'">Promedio de Duración de Ticket por Interfaz por Agente</h2>
          <br>
    ...
    methods: {
      ...
      async submit () {
        if (this.$refs.form.validate()) {
          this.initLoad()
          this.isLoading = true
          const res = await Api().post('/reports/agents/avg_time_ticket_interface', {
            agent_id: this.agent,
