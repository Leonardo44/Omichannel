    ...
    <v-layout row wrap>
      ...
      <v-flex xs12>
        <v-form ref="form" v-model="frmIsValid" lazy-validation>
          <h2 :class="colors.primary.text + ' text-xs-center text-sm-center text-md-center text-lg-center'">Promedio de Mensajes por Ticket por Interfaz.</h2>
          <br>
      ...
      <v-dialog v-model="resultCont" fullscreen hide-overlay transition="dialog-bottom-transition">
        <form ref="frmReportPDF" method="POST" action="http://172.16.11.172:9000/api/agent/averages_messages/pdf" target="__blank">
          <input type="hidden" name="data" :value="JSON.stringify(reportData)">
        </form>

        <form ref="frmReportExcel" method="POST" action="http://172.16.11.172:9000/api/agent/averages_messages/excel" target="__blank">
          <input type="hidden" name="data" :value="JSON.stringify(reportData)">
        </form>
        <v-card>
    ...
    methods: {
      ...
      async submit () {
        if (this.$refs.form.validate()) {
          this.initLoad()
          this.isLoading = true
          const res = await Api().post('/reports/agents/avg_msg_tickets', {
            agent_id: this.agent,
